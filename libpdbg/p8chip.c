...
static int p8_ram_getxer(struct pdbg_target *thread, uint64_t *value)
{
	...
	for (i = 0; i < 4; i++) {
		uint64_t opcodes[] = {mfxerf(0, i), mtspr(277, 0)};
	...
	*value = fields[0] | fields[1] | fields[2] | fields[3];
	...
	uint64_t opcodes[] = {mfxerf(0, 0), mtspr(277, 0), mfxerf(0, 1), mtspr(277, 0), mfxerf(0, 2), mtspr(277, 0), mfxerf(0, 3), mtspr(277, 0)};
	...
	printf("val %lx \n", results[5] );
	printf("val %lx \n", results[7] );
	printf("val %lx \n", results[1] |results[3] | results[5] | results[7]);
