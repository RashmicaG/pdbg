...
static void put_mem(uint64_t *stack, void *priv)
{
	...
	core = pdbg_target_parent(thread_target, "core");
	if (core == NULL)
	{
...
int handle_gdb(int optind, int argc, char *argv[])
{
	...
	char *end;
	uint16_t port = strtol(argv[optind +1], &end, 10);
	/* check port is in range */
	if (port > UINT16_MAX || port < 0 || *end != '\0') {
		printf("port needs to be a valid port number \n");
		return -1;
	}

	/* For the moment we assume all threads on the given core are stopped. Ideally we will change
	 * this to automatically do that but that requires some other changes. */
